...
- [x] **PG-39** Luồng tick không còn chờ client — đo được: **120 → 4.400 nhịp mỗi giây** với đúng một trình duyệt đang chơi ở ×100. Nguyên nhân: bản đồ thu nhỏ hỏi 128×128 = 16.384 ô mỗi 2,5 giây và `Game::tile` chạy lại **toàn bộ** worldgen cho mỗi ô — trong khi đang giữ khóa thế giới. Bảng nhớ cũ chỉ giữ `(cao độ, có nước)`, đủ cho tìm đường và không đủ cho `tile`. Giờ nhớ cả `BaseCell`. `§P6.8` quy tắc 2 chỉ đúng khi việc phục vụ client cũng rẻ. **Còn lại**: lần sinh **đầu** một vùng mới vẫn tốn ~1 giây ở bản debug.
- [x] **PG-40** Sự kiện đọc được thành câu — `econ.shortage` thành *"Làng thiếu food."*, `npc.intended` thành *"Định eat."*. Những dòng này đi thẳng vào câu trả lời của Yuu và vào biên niên sử, tức là vào chỗ người chơi **đọc**; `cause=9125 kind=food → consumed={6}` thì đúng mà không ai đọc nổi.
- [x] **PG-30** WebSocket thay HTTP-poll — `axum` + `tokio`, đẩy `state` mỗi 100 ms và **bỏ qua khung tin** khi `state_hash` không đổi. Đo trên trình duyệt: **0 yêu cầu HTTP trong 4 giây** khi đang chơi. HTTP-poll giữ lại làm đường lui — một trò chơi không chạy vì một proxy chặn WebSocket là một trò chơi hỏng vì lý do không liên quan tới nó.
- [x] **PG-31** Kinh tế chạm được vào cư dân — kho nước/lương/gỗ/công cụ/hạt giống/độ phì chạy mỗi ngày, vào `state_hash`, và lực lượng lao động đếm từ **cư dân thật**. Kho cạn thì đói tăng nhanh hơn; đói tới ngưỡng thì `npc.starving`. Kiểm trên thế giới đang chạy: tới ngày 35 kho về 0 và cả làng đói 100 — giờ vị thần **có thứ để cứu**. `§12`
...
- [x] **PG-34 Yuu** — trợ lý của True God, `§3.1` bước 2. Chạy thật trên thế giới đang sống: hỏi *"Vì sao kho lương đang cạn?"* thì nó trả lời *"Kho food hiện còn 0, vì: Làng thiếu food. → Hết ngày thứ 35"* kèm số thứ tự sự kiện bấm được. Mọi câu **phải** trích dẫn một sự kiện có thật; câu không truy được thì **bị cắt** và hiện ra ở mục riêng — giấu nó đi thì không ai gỡ lỗi được. **Còn thiếu**: chưa nối model (vai `yuu`); hiện chạy bằng `without_model`, thứ đọc thẳng đồ thị nhân quả và cũng là **đáy** mà mọi câu trả lời phải đứng trên.
- [~] **PG-35** Tri thức cục bộ — `mow-memory` đã có: ký ức chứng kiến/nghe kể, mờ dần theo loại (thần can thiệp nhớ lâu gấp bốn mươi lần một lần gặp mặt), sổ có trần bỏ **cái yếu nhất** chứ không phải cái cũ nhất, và quan hệ **suy ra từ ký ức** chứ không lưu riêng — để câu *"vì sao hai người này ghét nhau"* luôn trả lời được. 48 test. **Chưa nối** vào vòng chơi. — cư dân quan sát nhau, nhớ, và có quan hệ suy ra từ ký ức. `§1.2.2` nói *"một cá thể chỉ biết điều nó cảm nhận, được dạy, suy luận hoặc nghe kể"*; hiện chưa ai để ý tới ai. Đây là ba mắt còn thiếu của vòng lặp câu chuyện ở `§3.2`.
- [~] **PG-36** Thế giới thở — `render/liveliness.ts`: khói bếp (chỉ nhà có mái, chỉ đêm/rạng sáng), lúa lay, nước lấp lánh, nhịp bước và bóng theo hướng nắng. Lệch pha theo **toạ độ ô**, không `Math.random()`: cùng một thế giới phải trông giống nhau ở hai lần mở. Trần cứng 180 hình mỗi khung, tắt hết dưới mức phóng 9 px. 30 test. **Chưa tự chơi kiểm chứng** trên màn hình. — khói bếp, lúa lay, nhịp bước, bóng theo hướng nắng. Lệch pha theo toạ độ ô chứ không bằng `Math.random()`: cùng một thế giới phải trông giống nhau ở hai lần mở.
...
- [x] **PD-05** File `.js` dịch sẵn nằm cạnh `.ts` — `moduleResolution: "bundler"` ưu tiên `.js`, nên 92 file cũ do một lần chạy `tsc` để lại đã **che** mất chính nguồn của chúng. `@/i18n` giải ra bản `.js` thiếu khóa mới, `t("yuu.hint")` trả về chuỗi rỗng, và cả một panel hiện ra trống rỗng — không lỗi, không cảnh báo. Đã đặt `noEmit: true` để chặn nguồn, và thêm một bài test đọc thư mục làm lớp thứ hai. **Đây là lần thứ hai lỗi này cắn.**
...
- 2026-08-31 — 92 file `.js` cũ che mất `.ts` cùng tên: `t("yuu.hint")` trả chuỗi rỗng còn `import()` thẳng file `.ts` trả đúng chữ. `noEmit: true` + một bài test đọc thư mục — PD-05
- 2026-08-31 — Một trình duyệt đang chơi kéo luồng tick từ 4.480 xuống 120 nhịp/giây: bản đồ thu nhỏ hỏi 16.384 ô mỗi 2,5 giây, mỗi ô chạy lại worldgen dưới khóa. Nhớ cả `BaseCell` — 4.400 — PG-39
- 2026-08-31 — Yuu trả lời được trên thế giới thật, và mọi câu đều kèm số thứ tự sự kiện bấm được. Câu không chứng minh được thì bị cắt và **hiện ra** ở mục riêng — Yuu
